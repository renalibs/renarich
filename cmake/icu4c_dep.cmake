...
elseif(UNIX)
    string(APPEND ICU4C_PKG_URL "UNIX/GCC.tar.gz")
    string(APPEND ICU4C_PKG_PATH ".tar.gz")
    set(ICU4C_PKG_HASH "3d1d5e68fb58a462df1d80c7ce4c836b8dfb14e63851cad76cdfe88b914d6208")
else()
    message(FATAL_ERROR "Other platforms except Windows and Unix-like are not supported.")
endif()
...
if(WIN32)
    ...
    execute_process(
        COMMAND tar -zxvf ${ICU4C_PKG_PATH} -C ${ICU4C_BIN_DIR}
        COMMAND_ERROR_IS_FATAL ANY
    )
    set(ICU4C_ROOT_DIR "${ICU4C_BIN_DIR}/GCC" CACHE STRING INTERNAL)
    set(ICU4C_INCLUDE_DIR "${ICU4C_ROOT_DIR}/include" CACHE STRING INTERNAL)
    set(ICU4C_LIB_DATA "${ICU4C_ROOT_DIR}/lib/libicudata.so" CACHE STRING INTERNAL)
    set(ICU4C_LIB_DATA_FILE_LIST
        "${ICU4C_ROOT_DIR}/lib/libicudata.so"
        "${ICU4C_ROOT_DIR}/lib/libicudata.so.76"
        "${ICU4C_ROOT_DIR}/lib/libicudata.so.76.1"
    CACHE STRING INTERNAL)
    set(ICU4C_LIB_COMMON "${ICU4C_ROOT_DIR}/lib/libicuuc.so" CACHE STRING INTERNAL)
    set(ICU4C_LIB_COMMON_FILE_LIST
        "${ICU4C_ROOT_DIR}/lib/libicuuc.so"
        "${ICU4C_ROOT_DIR}/lib/libicuuc.so.76"
        "${ICU4C_ROOT_DIR}/lib/libicuuc.so.76.1"
    CACHE STRING INTERNAL)
    set(ICU4C_LIB_I18N "${ICU4C_ROOT_DIR}/lib/libicui18n.so" CACHE STRING INTERNAL)
    set(ICU4C_LIB_I18N_FILE_LIST
        "${ICU4C_ROOT_DIR}/lib/libicui18n.so"
        "${ICU4C_ROOT_DIR}/lib/libicui18n.so.76"
        "${ICU4C_ROOT_DIR}/lib/libicui18n.so.76.1"
    CACHE STRING INTERNAL)
    set(ICU4C_LIB_IO "${ICU4C_ROOT_DIR}/lib/libicuio.so" CACHE STRING INTERNAL)
    set(ICU4C_LIB_IO_FILE_LIST
        "${ICU4C_ROOT_DIR}/lib/libicuio.so"
        "${ICU4C_ROOT_DIR}/lib/libicuio.so.76"
        "${ICU4C_ROOT_DIR}/lib/libicuio.so.76.1"
    CACHE STRING INTERNAL)
    set(ICU4C_LIB_TOOL_UTILS "${ICU4C_ROOT_DIR}/lib/libicuuc.so" CACHE STRING INTERNAL)
    set(ICU4C_LIB_TOOL_UTILS_FILE_LIST
        "${ICU4C_ROOT_DIR}/lib/libicuuc.so"
        "${ICU4C_ROOT_DIR}/lib/libicuuc.so.76"
        "${ICU4C_ROOT_DIR}/lib/libicuuc.so.76.1"
