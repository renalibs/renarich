/**
 * This source code is based the implementation of wcwidth() and wcswidth() from Markus Kuhn,
 * downloaded from http://www.cl.cam.ac.uk/~mgk25/ucs/wcwidth.c.
 *
 * For code organise consistency I add new functions, namespace and move them into a C++ code file,
 * but the core implementation doesn't change.
 *
...
int rena::builtin::wcswidth( const wchar_t* __cp_wc_ws , std::size_t __ull_n ){
    return renarichxx_cfg::use_cjk_char_spacing() ? mk_wcswidth_cjk( __cp_wc_ws , __ull_n )
                                                  : mk_wcswidth( __cp_wc_ws , __ull_n );
}
...
int rena::builtin::wswidth( const std::wstring& __c_ws_str ){
    return wcswidth( __c_ws_str.c_str() , __c_ws_str.size() );
}
